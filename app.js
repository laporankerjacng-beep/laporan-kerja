...
function setupEventListeners() {
  ...
  document.getElementById('btnFlipCamera').addEventListener('click', handleFlipCamera);
  document.getElementById('btnFlipCameraBottom').addEventListener('click', handleFlipCamera);

  document.getElementById('btnSnapPhoto').addEventListener('click', snapPhoto);
  document.getElementById('btnRecordVideo').addEventListener('click', handleVideoRecordingClick);
  document.getElementById('btnConfirmMultiPhotos').addEventListener('click', submitAllCapturedMedia);

  document.getElementById('btnTabModePhoto').addEventListener('click', () => setCameraMode('photo'));
  document.getElementById('btnTabModeVideo').addEventListener('click', () => setCameraMode('video'));
...
async function finishVideoRecording() {
  ...
  if (blob && blob.size > 0) {
    ...
    reader.onloadend = () => {
      ...
      alert('Video berhasil direkam (' + Math.round(blob.size / 1024) + ' KB). Klik tombol hijau di bawah untuk menyimpan.');
    };
